...
const getByID = async (req, res) => {
    if (!ObjectId.isValid(req.params.id)) {
        res
          .status(400)
          .json("Must use a valid innjury id to find a injury.");
      }
...
const getByInjury = async (req, res) => {
    if (!ObjectId.isValid(req.params.injury)) {
        res
          .status(400)
          .json("Must use a valid injury to get injury.");
      }
      const userId = new ObjectId(req.params.id);
      mongodb
        .getDb()
...
const getByLength = async (req, res) => {
    if (!ObjectId.isValid(req.params.injury)) {
        res
          .status(400)
          .json("Must use a valid length to get injury.");
      }
      const userId = new ObjectId(req.params.id);
      mongodb
        .getDb()
    ...
    const newInjury = {
      ...
      if (response.acknowledged) {
        ...
        res
          ...
          .json(
            response.error ||
              "Some error occurred while creating the injury entry."
          );
      }
};
...
const updateInjuryReserve = async (req, res) => {
    if (!ObjectId.isValid(req.params.id)) {
        res
          .status(400)
          .json("Must use a valid injury id to update an injury.");
      }
      const userId = new ObjectId(req.params.id);
    ...
    if (!ObjectId.isValid(req.params.id)) {
      ...
      if (response.deletedCount > 0) {
        ...
        res
          ...
          .json(
            response.error ||
              "Some error occurred while deleting the injury."
          );
      }
};
 

module.exports = { getAll, getByID, getByInjury, getByLength, createInjuryReserve, updateInjuryReserve, deleteInjuryReserve};